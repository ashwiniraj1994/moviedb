...
public class MoviesResource extends BaseResource {
    ...
    public List<MovieEntity> moviesSearch(@QueryParam("q") String query,
                                            @DefaultValue("0") @QueryParam("pageNumber") int pageNumber,
                                            @DefaultValue("10") @QueryParam("pageSize") int pageSize) {
        LOGGER.info("Enter GET moviesSearch {}", query);
        List<MovieEntity> movies = new ArrayList<>();
        SearchVo obj = new SearchVo();
        obj.setQuery(query);
        obj.setPageNum(pageNumber);
        obj.setPageSize(pageSize);
        try {
            String q = query;
            if (StringUtils.isBlank(obj.getQuery())) {
                obj.setQuery("%");
            }

            movies = movieWebService.search(obj);

            return movies;
        } catch (ApplicationException ex) {
            LOGGER.error("Error while searching for Sites {}", ex);
            buildErrorMsgvo(Response.Status.INTERNAL_SERVER_ERROR, appProperties.getProperty(MOVIE_ERROR_CODE),
                    MOVIE, "", "", "", ex.getMessage(), "Error Searching movies");
        }
        return movies;
